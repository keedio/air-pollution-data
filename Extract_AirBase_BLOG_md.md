...
To extract the rawdata for the previous selection per country, a function called *extract\_CT\_data* extracts a country's rawdata based on the selected components and sample periods. This function is incorporated in the ***AirBase\_CT\_data.R*** script.
...
See Figure 1 for an example from the Spain data set, we selected Sulphur Dioxide component measured hourly (shaded area = selected data).

In order to be able to view the resulting dygra plot, we redirect the reader to the file dygraph\_blog.html
...
up to the end of the measured time period.

Then, with the previous information, we chop/slice/trim the extracted datasets. A function called *chop\_fill\_data* does this and fills NA values in the data (or not, the option is given). As before, this function is incorporated in the ***AirBase\_CT\_data.R*** script.
